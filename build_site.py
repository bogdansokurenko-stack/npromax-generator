...
def layout(title, desc, body, active='', canonical='', og_image='', og_type='website', og_extra=''):
    ...
    navlinks += f'<a href="orenda-kavomashyny.html" class="nav-a nav-rent{" active" if active=="orenda" else ""}">Оренда 0&nbsp;₴</a>'
    ...
    _gids=[x for x in (GA4_ID, GADS_ID) if x]
    if _gids:
        _cfg=''.join("gtag('config','%s');"%g for g in _gids)
        _ads=("window.__GADS='%s';window.__GADS_LEAD='%s';"%(GADS_ID,GADS_LEAD_LABEL)) if GADS_ID else ''
        gtag=("<script async src=\"https://www.googletagmanager.com/gtag/js?id=%s\"></script>\n"
              "<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}"
              "gtag('js',new Date());%s%s</script>\n")%(_gids[0],_cfg,_ads)
    else:
        gtag=''
...
{pixel}{gtag}<link rel="stylesheet" href="assets/style.css?v=12">
...
.nav-rent{color:var(--orange);font-weight:700}
.nav-rent:before{content:'●';font-size:9px;vertical-align:2px;margin-right:4px;color:var(--orange)}
...
.hr-vis{position:relative;justify-self:center;flex-shrink:0}
.hr-vis img{width:300px;height:300px;max-width:100%;object-fit:cover;border-radius:18px;display:block;background:#f4eee6;box-shadow:0 22px 50px rgba(0,0,0,.35)}
.hr-chip{position:absolute;left:-14px;bottom:14px;background:var(--orange);color:#fff;border-radius:14px;padding:10px 16px;font-size:24px;font-weight:800;line-height:1;box-shadow:0 14px 30px rgba(238,122,14,.45)}
.hr-chip small{font-size:12px;font-weight:600}
@media(max-width:820px){.home-rent{grid-template-columns:1fr;text-align:center;padding:32px 22px}.home-rent p{margin-left:auto;margin-right:auto}.hr-vis img{width:240px;height:240px}}
...
function oneClickBuy(e){e.preventDefault();var f=e.target;var fd=new FormData(f);
 ...
 leadSend('one_click',(function(){var o={name:fd.get('name')||'',phone:fd.get('phone')||'',
   product:(window.__pdTitle||'')+' · '+(window.__pdVariant||'')+' · '+(window.__pdPrice||'')+' грн (арт. '+(window.__pdSku||'')+')',
   comment:'К-сть: '+((document.getElementById('pdQty')||{}).value||1)};
   TRACK_KEYS.forEach(function(k){var v=getTrack(k);if(v)o[k]=v;});return o;})(),
  'Купити в 1 клік: '+(window.__pdTitle||''))
...
function leadCapture(e){e.preventDefault();var f=e.target;var fd=new FormData(f);
 ...
 leadSend('consult',(function(){var o={phone:fd.get('phone')||'',name:fd.get('name')||''};
   TRACK_KEYS.forEach(function(k){var v=getTrack(k);if(v)o[k]=v;});return o;})(),'Заявка на консультацію (головна)')
...
// NPX-037: лід у власний бекенд (БД + email + Telegram). Резерв — formsubmit, щоб заявка з платного трафіку не загубилась.
function leadSend(type,data,subject){
 var body=Object.assign({type:type,page:location.pathname+location.search},data||{});
 var fs=function(){return fsSend(subject||('Заявка ('+type+') — npromax.com.ua'),data||{});};
 return fetch('/api/lead.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
  .then(function(r){ if(!r.ok) throw new Error('lead http '+r.status); return r.json(); })
  .then(function(j){ if(!j||!j.ok) throw new Error('lead bad'); if(!j.stored){ try{fs();}catch(e){} } return j; })
  .catch(function(){ return fs(); });}
...
// Google Ads: конверсія (спрацьовує лише якщо задано GADS_ID + мітку)
function gAdsConv(label,params){try{if(typeof gtag==='function'&&window.__GADS&&label)gtag('event','conversion',Object.assign({send_to:window.__GADS+'/'+label},params||{}));}catch(e){}}
...
function submitOrder(e){e.preventDefault();var c=getCart();if(!c.length){toast('Кошик порожній');return;}
 var f=e.target;var fd=new FormData(f);
 var items=c.map(i=>({slug:i.slug,sku:i.sku||'',title:i.title,variant:i.variant||'',price:i.price,qty:i.qty}));
 var total=c.reduce((s,i)=>s+i.price*i.qty,0);
 var btn=f.querySelector('button[type=submit]');btn.disabled=true;btn.textContent='Відправляємо…';
 var payload={name:fd.get('name')||'',phone:fd.get('phone')||'',city:fd.get('city')||'',np_office:fd.get('np')||'',payment:fd.get('payment')||'',items:items,total:total,utm_source:getTrack('utm_source'),utm_medium:getTrack('utm_medium'),utm_campaign:getTrack('utm_campaign'),fbclid:getTrack('fbclid')};
 function finish(num){
  try{sessionStorage.setItem('npromax_last_order',JSON.stringify({id:num,value:total,items:c.map(i=>({item_id:i.sku||i.slug,item_name:i.title,price:i.price,quantity:i.qty}))}));}catch(err){}
 ...
 // основне: замовлення в адмінку (БД) через /api/order.php; резерв — лист formsubmit, щоб не втратити замовлення
 fetch('/api/order.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
  .then(r=>r.json()).then(function(res){if(res&&res.ok&&res.order_no){finish(res.order_no);}else{throw new Error('api');}})
  .catch(function(){var num=orderNum();var itemsTxt=c.map(i=>i.title+' ['+(i.variant||'')+(i.sku?', арт. '+i.sku:'')+'] x'+i.qty+' = '+(i.price*i.qty)+' грн').join('; ');
   fsSend('Замовлення '+num+' з сайту npromax.com.ua',{order:num,name:payload.name,phone:payload.phone,city:payload.city,np_office:payload.np_office,payment:payload.payment,items:itemsTxt,total:total+' грн'})
    .then(()=>finish(num)).catch(()=>{btn.disabled=false;btn.textContent='Оформити замовлення';toast('Не вдалося відправити. Напишіть на info@npromax.com.ua');});});}
...
function submitB2B(e){e.preventDefault();var f=e.target;var fd=new FormData(f);
 ...
 data.place=data.business||'';
 leadSend('b2b',data,'B2B заявка з npromax.com.ua')
...
var TRACK_KEYS=['utm_source','utm_medium','utm_campaign','utm_content','utm_term','fbclid','gclid','wbraid','gbraid'];
...
function submitRental(e){e.preventDefault();var f=e.target;var fd=new FormData(f);
 ...
 leadSend('rental',data,'Заявка на оренду POLTI ('+color+') — npromax.com.ua')
...
def b2b_page(cards):
    ...
    blocks=[('Рішення для офісу','Зернова кава для кавомашин, капсули та монодози — щоб колеги завжди мали смачну чашку.'),
            ...
            ('Оренда кавомашини','POLTI Coffea під чалди E.S.E. 44 мм: 0 ₴/міс від 600 чалдів на місяць, інакше 1000 ₴/міс.'),
...
def rent_faq(decaf_link):
 ...
 ('Чи підходять капсули Nespresso, Dolce Gusto, Lavazza?','Ні. POLTI Coffea працює лише зі стандартними паперовими чалдами E.S.E. 44 мм. Капсульні системи не сумісні.'),
...
def _rent_form(color_key):
    ...
    utm=''.join(f'<input type="hidden" name="{k}" id="f_{k}">' for k in ['utm_source','utm_medium','utm_campaign','utm_content','utm_term','fbclid','gclid','wbraid','gbraid'])
  ...
  <div class="form-row"><label>Тип закладу</label><select name="place" required><option value="" disabled selected>Оберіть тип</option><option>Офіс</option><option>Кафе / кав’ярня</option><option>Магазин</option><option>Салон краси / барбершоп</option><option>Автомийка / СТО</option><option>Готель / апартаменти</option><option>Інше</option></select></div>
...
# ---- Google: GA4 + Ads. Порожньо = тег НЕ рендериться (нічого не ламається без ID) ----
GA4_ID = ''            # напр. 'G-XXXXXXXXXX' (GA4 → Потоки даних)
GADS_ID = ''           # напр. 'AW-123456789' (Google Ads → Конверсії)
GADS_LEAD_LABEL = ''   # мітка конверсії ліда, напр. 'AbC-D_efGh12'  → send_to = GADS_ID/LABEL
...
def _rent_form_hub():
    ...
    utm=''.join(f'<input type="hidden" name="{k}" id="fh_{k}">' for k in ['utm_source','utm_medium','utm_campaign','utm_content','utm_term','fbclid','gclid','wbraid','gbraid'])
  ...
  <div class="form-row"><label>Тип закладу</label><select name="place" required><option value="" disabled selected>Оберіть тип</option><option>Офіс</option><option>Дім</option><option>Кафе / кав’ярня</option><option>Магазин</option><option>Салон краси / барбершоп</option><option>Інше</option></select></div>
  <div class="form-row"><label>Місто</label><input name="city" placeholder="Місто"></div>
  <div class="form-row"><label>Орієнтовно чашок на день</label><input name="cups" inputmode="numeric" placeholder="Напр. 20"></div>
  <input type="text" name="_honey" style="display:none" tabindex="-1" autocomplete="off">
  <label class="f-opt" style="margin:4px 0 10px"><input type="checkbox" required> Погоджуюсь з <a href="privacy.html" style="color:var(--orange)">політикою конфіденційності</a></label>
...
<section class="section" id="how" style="background:var(--crema)"><div class="wrap">
  ...
  <div class="steps">
    <div class="step"><span class="step-n">1</span><h3>Обираєте колір</h3><p>Біла (S18W) або чорна (S15B) POLTI Coffea — під ваш інтер’єр.</p></div>
...
<section class="section" style="background:var(--soft)"><div class="wrap">
  <div class="section-h"><h2>Характеристики {c['model']}</h2><div class="sub">Компактна, вузька (лише 10,5 см), проста в користуванні</div></div>
...
'''
    ...
    desc=f"Оренда {c['model']} ({c['name']}) — 0 ₴/міс від 600 чалдів E.S.E. на місяць, інакше 1000 ₴. Договір, доставка по Україні."
...
HUB_FAQ_BASE=[
 ...
 ('Який колір можна обрати?','Два апарати на вибір — біла POLTI Coffea S18W і чорна POLTI Coffea S15B. Оберіть у формі заявки.'),
...
def rental_hub(cards):
    ...
    svc_ld=json.dumps({"@context":"https://schema.org","@type":"Service",
      "name":"Оренда кавомашини POLTI Coffea для чалдів E.S.E.","serviceType":"Оренда кавомашини",
      "provider":{"@type":"Organization","name":"NPROMAX","url":B},"areaServed":{"@type":"Country","name":"Україна"},
      "image":B+"assets/img/polti-bila.jpg",
      "description":"Безкоштовна оренда кавомашини POLTI Coffea (біла S18W або чорна S15B) за умови купівлі від 600 чалдів E.S.E. на місяць; інакше 1000 грн/міс. Апарат лишається власністю NPROMAX (договір оренди).",
...
<section class="rh-hero"><div class="rh-hero-in">
  ...
  <div class="rh-hero-vis">
    <div class="rh-frame">
      <picture><source srcset="assets/img/polti-bila.webp" type="image/webp"><img src="assets/img/polti-bila.jpg" alt="Кавомашина POLTI Coffea S18W (біла) для чалдів E.S.E." width="900" height="900" fetchpriority="high"></picture>
...
'''
    ...
    return layout('Оренда кавомашини POLTI Coffea для чалдів E.S.E. — 0 грн/міс | NPROMAX',
      'Оренда кавомашини POLTI Coffea під чалди E.S.E.: 0 ₴/міс від 600 чалдів на місяць, інакше 1000 ₴. Доставка по Україні.',
...
<script>
(function(){try{
  var raw=sessionStorage.getItem('npromax_lead');
  if(!raw) return;
  if(sessionStorage.getItem('npromax_lead_sent')) return;
  var d={}; try{d=JSON.parse(raw)||{};}catch(e){}
  var p={form:d.type||'lead',color:d.color||'',currency:'UAH',value:0};
  try{window.dataLayer=window.dataLayer||[];window.dataLayer.push(Object.assign({event:'generate_lead'},p));}catch(e){}
  try{if(typeof gtag==='function')gtag('event','generate_lead',p);}catch(e){}
  try{if(typeof gtag==='function'&&window.__GADS&&window.__GADS_LEAD)gtag('event','conversion',{send_to:window.__GADS+'/'+window.__GADS_LEAD,value:0,currency:'UAH'});}catch(e){}
  sessionStorage.setItem('npromax_lead_sent','1');
}catch(e){}})();
</script>'''
...
# ===== SHOP BACKEND (Phase 1: PHP admin+orders). Source: _generator/php_src/*.php =====
# Embedded base64 (byte-exact, no-secret). Regenerate after editing php_src.
import base64 as _b64
_SHOP_PHP = {
  '_shopcore.php': '',
  'api/order.php': '',
  'api/lead.php': '',
  'admin.php': '',
}
def write_shop_backend(site):
    for _rel,_b in _SHOP_PHP.items():
        _p=os.path.join(site,_rel)
        os.makedirs(os.path.dirname(_p),exist_ok=True)
        open(_p,'wb').write(_b64.b64decode(_b))
